...
class Quote_Manager
{
    ...
    public static function clear_history( array $currencies = null )
    {
        ...
        $query = "delete from $table_name where created_by = " . get_current_user_id();
        if ( ! is_null( $currencies ) ) {
            if ( empty ( $currencies ) ) {
                return;
            }
    ...
    public static function values( $date )
    {
        ...
        foreach ( self::currencies() as $currency ) {
            $query[] = $wpdb->prepare(
                "(select currency, value from $table_name where created_by = " . get_current_user_id() . " AND currency = %s and date <= %s order by date desc limit 1)",
                $currency->code,
                $date
            );
        }
    ...
    protected static function save( $currency, $date, $value )
    {
        ...
        $created_by = get_current_user_id();  // Get current user ID
        $updated = $wpdb->update(
            $table_name,
        ...
        if ( ! $updated ) {
            $wpdb->insert(
                $table_name,
                compact( 'currency', 'date', 'value', 'created_at', 'updated_at', 'created_by' ),
                array( '%s', '%s', '%f', '%s', '%s', '%d' )
            );
        }
    }
    ...
    protected static function base_currency()
    {
        ...
        if ( ! $base_currency ) {
            $items = Models\Currency::get_results( function ( $query ) {
                return $query . ' where created_by = ' . get_current_user_id() . ' AND is_base = 1';
            } );
    ...
    protected static function currencies()
    {
        ...
        if ( ! $currencies ) {
            $currencies = Models\Currency::get_results(function ( $query ) {
                return $query . ' where created_by = ' . get_current_user_id();
            } );
        }
