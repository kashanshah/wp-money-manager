...
class App_Controller extends Base_Controller
{
    ...
    public function load_app( WP_REST_Request $request )
    {
        $accounts = Account::rows( function ( $query ) {
            return $query . ' WHERE created_by = ' . get_current_user_id() . ' order by field(type,"checking","card","cash","debt"), title';
        } );
        $categories = Category::rows( function ( $query ) {
            return $query . ' WHERE created_by = ' . get_current_user_id() . ' order by title';
        } );
        $currencies = Currency::rows( function ( $query ) {
