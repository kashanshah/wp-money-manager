    ...
    public function save_currency( WP_REST_Request $request )
    {
        ...
        if ( isset ( $input['id'] ) ) {
            ...
            $duplicates = Currency::get_results( function ( $query ) use ( $wpdb, $input ) {
                return $query . $wpdb->prepare( ' where created_by = ' . get_current_user_id() . ' AND code = %s', $input['code'] );
            } );
            if ( ! empty ( $duplicates ) ) {
        ...
        if ( $changing_base_currency ) {
            ...
            $wpdb->query( "update $table_name set is_base = 0 WHERE created_by = " . get_current_user_id() );
        }
        $currency->save();
