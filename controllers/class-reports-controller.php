    ...
    protected function get_report_data( $currency, $range, $group_by, $account_id, $include_transfers )
    {
            ...
            switch ( $range ) {
                case 'recent_12_months':
                    $date_condition = $wpdb->prepare(
                        't.date >= %s and t.created_by = ' . get_current_user_id(),
                        current_datetime()->modify( 'first day of 11 months ago' )->format( 'Y-m-d' )
                    );
                    break;
                case 'this_year':
                    $date_condition = $wpdb->prepare(
                        't.date between %s and %s and t.created_by = ' . get_current_user_id(),
        ...
        if ( $account_id !== null ) {
            $account_condition = $wpdb->prepare(
                '(t.account_id = %1$d or t.to_account_id = %1$d) AND t.created_by = ' . get_current_user_id(),
                $account_id
            );
        }
        // Apply grouping
            ...
            from (
                select
                    ...
                    (
                        ...
                        where q1.currency = c1.code and q1.date <= t.date and q1.created_by = ".get_current_user_id()." and c1.created_by = ".get_current_user_id()." 
                        order by q1.date desc
                        limit 1
                    ) as quote1,
                    (
                        select q2.value from $quotes_table_name q2
                        where q2.currency = c2.code and q2.date <= t.date and q2.created_by = ".get_current_user_id()." and c2.created_by = ".get_current_user_id()." 
                ...
                where (t.type = %s or t.type = %s and (t.account_id = %d or t.to_account_id = %d))
                    ...
                    and a.created_by = ".get_current_user_id()."
                    and c1.created_by = ".get_current_user_id()."
                    and c2.created_by = ".get_current_user_id()."
            ) tmp
