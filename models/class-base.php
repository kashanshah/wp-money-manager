...
class Base
{
    ...
    public function save()
    {
        ...
        if ( isset ( $this->data['id'] ) ) {
            return $wpdb->update(
                static::table_name(),
                array( 'updated_at' => $now, 'created_by' => get_current_user_id() ) + static::normalize_data( array_diff_key( $this->data, array( 'id' => false ) ) ),
                array( 'id' => $this->data['id'] )
            );
        } else {
            $result = $wpdb->insert(
                static::table_name(),
                array( 'created_at' => $now, 'updated_at' => $now, 'created_by' => isset($this->data['created_by']) ? $this->data['created_by'] : get_current_user_id() ) + static::normalize_data( $this->data )
            );
            if ( $result ) {
    ...
    public static function find( $id )
    {
        ...
        $models = static::get_results( function ( $query ) use ( $wpdb, $id ) {
            return $query . $wpdb->prepare( ' where created_by = '. get_current_user_id() .' AND id = %d', $id );
        }, true );
    ...
    public static function destroy( $ids )
    {
        ...
        $query = "delete from $table_name where created_by = " . get_current_user_id() . " AND id in ($ids_placeholders)";
